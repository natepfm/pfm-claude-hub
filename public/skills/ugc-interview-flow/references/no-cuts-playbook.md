...
1. Seedance 2.0, 720p, ≤15s clips, `--start-image` (never `--image` — EXCEPT the phone-screen closer, lesson 10: lander as `--image` + 1080p), 9:16.
...
10. **Phone screens render CRISP IN-CLIP — never a post comp** (method updated 2026-07-24, Mitchell Gamache; proven on Home Block Party British, all 4 versions' c6 closers, 07-23). Two moves together: (a) pass the **brandless lander PNG as an `--image` reference** alongside the seam frame as `--start-image`, and (b) fire the closer at **1080p (`mode std`)**. Seedance then renders the results page — e.g. a "$550/year" — sharp and legible directly in the clip. Keep the content lock in the prompt: name the actual elements ("the large $550/year, the button — SHARP, STEADY, square to camera") + no-blur/no-garble/no-invented-interface negatives; the lander ref and the content lock work together, not instead of each other. 🔴 **It is never the editor's job to comp the screen in post.** A garbled screen means re-fire at 1080p with the lander ref — never hand off a comp job. (Supersedes the old guidance that small print garbles at 720p and that ceiling is a post screen-comp.)
