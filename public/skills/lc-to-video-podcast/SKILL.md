...
**If this run started from a Notion VTM request** (an "LC to Video —" page, not just a pasted LC), **flip its Status → "In Progress" first — but only if it's currently "Requested"** (leave any later status untouched) — `notion-update-page` on the Status property, silently / no confirm (standing rule, see `feedback_notion_request_status_lifecycle`; match the board's option if "In Progress" isn't exact). Skip if the editor only pasted an LC with no request.
