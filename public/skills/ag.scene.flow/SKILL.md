...
8. **🔴 PROSE NEVER RE-DESCRIBES WHAT A REFERENCE SHOWS (07-31).** `assemble` is LEAN by default: when the shot's camera has a `reference_frame` on disk, the geography / environment / background_population / grade verbatim blocks are SUPPRESSED and replaced by one deixis line — the image shows them, and prose that repeats them competes with the pixels and wins (measured on the ComfyUI arm: a 3,557-char prompt could not hold a room that 72 chars + the same image held perfectly). Identity blocks are never suppressed — a wide plate carries no face. `--verbose` forces the old full form for the FIRST plate of a room, where prose is the only channel. The mode and char count print on every assemble.
9. **🔴 CAMERA LINES ARE POSITIONAL, NOT TARGET-BASED.** "Face the jury box wall" makes the model move the JURY BOX into the current view; "move the camera into the open well, turn left, looking at the jury box" moves the camera. Order is POSITION → DIRECTION → OBJECT. Lint REFUSES target-only phrasing on `unboarded` shots and warns on already-boarded ones.
10. **🔴 OCCLUDED DETAIL SHIPS AS PIXELS, NOT PROSE — `detail_refs`.** When a new camera needs an object the source frame never resolved (a jury box seen edge-on in the hero, shot square-on here), crop that region out of the canonical source and add `detail_refs: [{path, region_of}]` to the shot. `fire_frame.py` appends it after the masters and scopes its role to that object only. `region_of` is mandatory — an unscoped crop competes with image 1 for the whole scene.
11. **🔴 SET `scene.aspect` FIRST — every fire derives from it.** The bible declares the creative's aspect ONCE (`scene: aspect: "9:16"` or `"16:9"`; `init --aspect 16:9` sets it). Both fire scripts read it: frames render at it, clips render at it, and a camera's reference frame or a clip's start/end frame is REFUSED unless its real pixel aspect matches. **The law is ref aspect == render aspect, at ANY aspect** — a 16:9 plate in a 9:16 render (or the reverse) stretches anatomy and rescales furniture. So a CTV / Roku / podcast creative sets `16:9` and cuts 16:9 camera references; a story ad leaves the 9:16 default. Camera field is `reference_frame` (legacy `reference_frame_9x16` still honoured).
